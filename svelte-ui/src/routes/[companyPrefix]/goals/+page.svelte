...
<div class="space-y-6 p-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold text-[#F8FAFC]">Goals</h1>
      <p class="mt-1 text-sm text-[#94A3B8]">Track objectives and key results</p>
    </div>
    <button
  ...
  {#if showCreate}
    ...
    >
      <div>
        <label for="goal-title" class="block text-sm font-medium text-[#F8FAFC] mb-1">Goal Title</label>
        <input
          id="goal-title"
          bind:value={newTitle}
  ...
  {:else if goals.length === 0}
    ...
    <div class="flex flex-col items-center justify-center py-20">
      <div class="rounded-full bg-white/[0.05] p-4 mb-4">
        <Target class="h-10 w-10 text-[#94A3B8]" />
      </div>
      <h3 class="text-lg font-medium text-[#F8FAFC]">No goals defined</h3>
      <p class="mt-1 text-sm text-[#94A3B8]">Set goals to align your team's efforts</p>
      <button
    ...
    <div class="space-y-3">
      {#each goals as goal (goal.id)}
        <a
          ...
          class="group flex items-center gap-4 rounded-xl border border-white/[0.08] bg-[#121218] p-4 transition-colors hover:bg-white/[0.03] hover:border-white/[0.12]"
        >
          <!-- Icon -->
          ...
          <div class="min-w-0 flex-1">
            ...
            {#if goal.progress !== undefined}
              <div class="flex items-center gap-3">
                <div class="h-1.5 flex-1 rounded-full bg-white/[0.08] overflow-hidden">
                  <div
                    class="h-full rounded-full transition-all {progressColor(goal.progress)}"
                    style="width: {Math.min(100, goal.progress)}%"
                  ></div>
                </div>
                <span class="shrink-0 text-xs font-medium text-[#94A3B8]">{goal.progress}%</span>
              </div>
            {/if}
