<script lang="ts">
  ...
  // All providers appear in the Quotas section; those without API quota windows
  // fall back to subscription / monthly-spend data.
  let providersWithQuota = $derived(providers);
      ...
      <div class="rounded-xl border border-border bg-card p-5">
        ...
        <div class="space-y-5">
          {#each providersWithQuota as provider (provider.adapterType)}
            {@const hasApiQuota = provider.quotaWindows && provider.quotaWindows.length > 0}
            {@const hasSubscription = provider.subscriptionLimitMonthly != null && provider.subscriptionLimitMonthly > 0}
            {@const spendCents = provider.monthlySpendCents ?? 0}
            {@const limitCents = (provider.subscriptionLimitMonthly ?? 0) * 100}
            {@const spendPct = limitCents > 0 ? Math.min(100, Math.round((spendCents / limitCents) * 100)) : 0}
            <div class="space-y-3">
              <!-- Provider header row -->
              <div class="flex items-center gap-2">
                <ProviderIcon adapterType={provider.adapterType} size={14} />
                <span class="text-sm font-semibold text-foreground">{provider.displayName}</span>
                {#if provider.subscriptionPlan}
                  <span class="inline-flex rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground uppercase tracking-wide">{provider.subscriptionPlan}</span>
                {/if}
              </div>

              <!-- API quota windows (e.g. Claude rate-limit buckets) -->
              {#if hasApiQuota}
                <div class="pl-5 space-y-2">
                  {#each provider.quotaWindows as window}
                    ...
                    <div class="space-y-1">
                      ...
                      {#if window.usedPercent != null}
                        <div class="h-1.5 w-full rounded-full {quotaBarBgColor(pct)} overflow-hidden">
                          <div
              ...
              {#if hasSubscription}
                ...
                {@const limitDollars = (provider.subscriptionLimitMonthly ?? 0).toFixed(0)}
                <div class="pl-5 space-y-1">
                  <div class="flex items-center justify-between">
                    <span class="text-xs text-muted-foreground">Monthly spend</span>
                    <div class="flex items-center gap-3 text-xs text-muted-foreground">
                      <span class="font-mono">${spendDollars} / ${limitDollars}</span>
                      <span class="font-medium {spendPct >= 90 ? 'text-red-500' : spendPct >= 70 ? 'text-yellow-500' : 'text-emerald-500'}">{spendPct}%</span>
                      {#if provider.subscriptionResetsAt}
                        <span class="inline-flex items-center gap-1">
                          <Clock class="h-3 w-3" />
                          {new Date(provider.subscriptionResetsAt).toLocaleDateString()}
                        </span>
                      {/if}
                    </div>
                  </div>
                  <div class="h-1.5 w-full rounded-full {quotaBarBgColor(spendPct)} overflow-hidden">
                    <div
                      class="h-full rounded-full transition-all duration-500 {quotaBarColor(spendPct)}"
              ...
              <!-- Neither quota windows nor subscription limit -->
              {#if !hasApiQuota && !hasSubscription}
                <div class="pl-5">
                  {#if provider.connectionStatus === 'connected' || provider.connectionStatus === 'degraded'}
                    <span class="text-xs text-muted-foreground">No rate limits configured — unlimited usage</span>
                  {:else}
                    <span class="text-xs text-muted-foreground italic">Provider not connected</span>
                  {/if}
                </div>
              {/if}
            ...
            {#if provider !== providersWithQuota[providersWithQuota.length - 1]}
