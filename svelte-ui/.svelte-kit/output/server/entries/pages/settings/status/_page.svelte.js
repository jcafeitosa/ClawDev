...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$renderer2.push(`<a${attr("href", tab.href)}${attr_class(`text-sm ${stringify(tab.href === "/settings/status" ? "font-medium text-indigo-600 dark:text-indigo-400" : "text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300")}`)}>${escape_html(tab.label)}</a>`);
    }
    $$renderer2.push(`<!--]--></div> <div class="flex items-center gap-3">`);
    Server($$renderer2, { class: "h-6 w-6 text-zinc-400" });
    $$renderer2.push(`<!----> <h1 class="text-xl font-bold text-zinc-900 dark:text-zinc-50">Instance Status</h1></div> `);
    {
      $$renderer2.push("<!--[0-->");
      ...
      for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
        ...
        $$renderer2.push(`<div class="h-40 animate-pulse rounded-lg bg-zinc-100 dark:bg-zinc-800"></div>`);
      }
      $$renderer2.push(`<!--]--></div>`);
    }
    $$renderer2.push(`<!--]--></div>`);
