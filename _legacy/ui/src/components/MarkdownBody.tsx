...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`clawdev-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="clawdev-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("clawdev-mermaid-status", error && "clawdev-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="clawdev-mermaid-source">
...
export function MarkdownBody({ children, className, resolveImageSrc }: MarkdownBodyProps) {
        ...
        return (
          <a
            ...
            className={cn(
              ...
              `clawdev-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "clawdev-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "clawdev-markdown prose prose-sm max-w-none break-words overflow-hidden",
