# ClawDev Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for ClawDev's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main ClawDev app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary ClawDev HTTP APIs directly.
...
ClawDev plugin design is based on the following assumptions:

1. ClawDev is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core ClawDev business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by ClawDev core.
...
2. Let plugins add major capabilities without editing ClawDev core.
...
4. Depend on project-local plugin folders such as `.clawdev/plugins`.
...
The single ClawDev deployment an operator installs and controls.

### 5.2 Company

A first-class ClawDev business object inside the instance.
...
A trusted in-process extension loaded directly by ClawDev core.
...
An installable instance-wide extension package loaded through the ClawDev plugin runtime.
...
ClawDev has two extension classes.
...
ClawDev already has a concrete workspace model:
...
Plugins live under the ClawDev instance directory.

Suggested layout:

- `~/.clawdev/instances/default/plugins/package.json`
- `~/.clawdev/instances/default/plugins/node_modules/`
- `~/.clawdev/instances/default/plugins/.cache/`
- `~/.clawdev/instances/default/data/plugins/<plugin-id>/`
...
ClawDev should add CLI commands:

- `pnpm clawdev plugin list`
- `pnpm clawdev plugin install <package[@version]>`
- `pnpm clawdev plugin uninstall <plugin-id>`
- `pnpm clawdev plugin upgrade <plugin-id> [version]`
- `pnpm clawdev plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@clawdev/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface ClawDevPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that ClawDev agents can use during runs.
...
- ClawDev server starts one worker process per installed plugin
...
Receives one typed ClawDev domain event.
...
export interface PluginContext {
  manifest: ClawDevPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@clawdev/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@clawdev/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@clawdev/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@clawdev/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@clawdev/plugin-sdk/ui`
...
The `@clawdev/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the ClawDev secret provider system rather than a plain text input
...
1. Core ClawDev data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing ClawDev objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual ClawDev product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if ClawDev later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing ClawDev secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm clawdev plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the ClawDev server. This is a normative requirement, not optional.
...
### 27.1 `@clawdev/plugin-test-harness`
...
```ts
import { createTestHarness } from "@clawdev/plugin-test-harness";
...
For developing a plugin against a running ClawDev instance:

- The operator installs the plugin from a local path: `pnpm clawdev plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-clawdev-plugin`) that scaffolds:

- `package.json` with correct `clawdevPlugin` keys
...
- `@clawdev/plugin-workspace-files`
- `@clawdev/plugin-terminal`
- `@clawdev/plugin-git`
- `@clawdev/plugin-linear`
- `@clawdev/plugin-github-issues`
- `@clawdev/plugin-grafana`
- `@clawdev/plugin-runtime-processes`
- `@clawdev/plugin-stripe`
...
- `@clawdev/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@clawdev/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@clawdev/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@clawdev/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@clawdev/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@clawdev/plugin-sdk/ui`
...
- `@clawdev/plugin-test-harness`
- `create-clawdev-plugin` starter template
...
ClawDev should not implement a generic in-process hook bag modeled directly after local coding tools.

ClawDev should implement:
...
That is the complete target design for the ClawDev plugin system.
