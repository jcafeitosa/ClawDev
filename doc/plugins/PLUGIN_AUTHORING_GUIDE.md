...
This guide describes the current, implemented way to create a ClawDev plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main ClawDev app.
...
```bash
pnpm --filter @clawdev/create-clawdev-plugin build
node packages/plugins/create-clawdev-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the ClawDev repo:

```bash
pnpm --filter @clawdev/create-clawdev-plugin build
node packages/plugins/create-clawdev-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/clawdev/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@clawdev/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@clawdev/plugin-sdk` from the local ClawDev checkout into a `.clawdev-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into ClawDev from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@clawdev/plugin-sdk/ui`
