...
This spec defines how ClawDev actually runs agents while staying runtime-agnostic.
...
1. ClawDev is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, ClawDev manages lifecycle, and on exit ClawDev parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `clawdev.skill` (shared ClawDev skill text block)
...
2. Preferred transport is env vars (`CLAWDEV_*`) injected at runtime.
