...
For `claude_local` and `codex_local`, ClawDev assumes the CLI is already installed and authenticated on the host machine.
...
ClawDev stores resumable session state per `(agent, taskKey, adapterType)`.
