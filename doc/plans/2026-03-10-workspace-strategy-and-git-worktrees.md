...
`PAP-447` asks how ClawDev should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same ClawDev instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into ClawDev:

- some operators use ClawDev to manage ClawDev and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as ClawDev
- Claude and Codex expose different built-in affordances, so ClawDev should not overfit to one tool

## Core Product Decision

ClawDev should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by ClawDev
...
- ClawDev should not assume that every adapter can see or use a host filesystem path directly
...
If ClawDev makes worktrees agent-first, it will blur:
...
- ClawDev core resolves and records execution workspace state
...
That gives ClawDev room to support:
...
- ClawDev handles workspace runtime services under the hood using project/workspace policy
...
Once ClawDev is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that ClawDev cannot infer
...
- when issue moves to done, should ClawDev auto-commit?
- should it auto-push?
...
ClawDev already has the right foundation for a project-first model.
...
For server-spawned local adapters, ClawDev already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that ClawDev should mirror directly

Implication:

- for `codex_local`, ClawDev should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but ClawDev should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, ClawDev should separate:
...
ClawDev should be able to express intentions such as:
...
These are not specific to ClawDev. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".clawdev/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a ClawDev-specific `devServer` flag.
...
ClawDev should distinguish between:
...
ClawDev should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, ClawDev should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to ClawDev
...
For runtime services specifically, ClawDev should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as ClawDev-only dev-server flags
...
3. Inject resolved execution workspace details into `context.clawdevWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / ClawDev instance can be reused through env/config injection when appropriate
...
- ClawDev can decide whether to reuse or start a fresh service deterministically
...
3. Test against ClawDev-on-ClawDev workflows first.
...
7. Workspace runtime services are modeled generically, not as ClawDev-specific dev-server toggles.
