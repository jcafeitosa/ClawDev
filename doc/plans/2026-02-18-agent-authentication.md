...
Agents need API keys to authenticate with ClawDev. The current approach
...
**Trust model:** The adapter process runs on the same machine as the ClawDev
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** ClawDev generates a token and passes it directly to the agent
...
`CLAWDEV_API_URL`, `CLAWDEV_AGENT_ID`, `CLAWDEV_COMPANY_ID`. We need to
add a `CLAWDEV_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a ClawDev CLI
...
```
clawdev auth login
# Opens browser -> user confirms -> token stored at ~/.clawdev/credentials
```
...
managed by the ClawDev server itself.
...
1. A company admin (user or agent) generates an **invite URL** from ClawDev.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The ClawDev SKILL.md (or a link to it)
   - What information ClawDev needs from the agent (e.g. webhook URL, adapter
...
5. ClawDev stores the pending registration.
...
7. On approval, ClawDev provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** ClawDev issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.clawdev.ing/skills/clawdev/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How ClawDev should send you heartbeats",
...
1. Generate a ClawDev invite link for the company.
...
4. A ClawDev company member approves the new agent.
5. ClawDev begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `CLAWDEV_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `clawdev auth login` for developer-managed remote agents.                                      |
