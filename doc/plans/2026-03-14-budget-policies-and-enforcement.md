...
ClawDev already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model ClawDev should implement next.

## Product Goals

ClawDev should let operators:
...
For the next implementation pass, ClawDev should enforce these defaults:
...
ClawDev should separate subscription-included usage from billed spend:
...
ClawDev should have two threshold classes:
...
ClawDev must know that a project is budget-paused and enforce it.
...
ClawDev must also block execution before new work starts.
