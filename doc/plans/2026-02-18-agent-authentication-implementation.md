...
- New config/env settings:
  - `CLAWDEV_AGENT_JWT_SECRET`
  - `CLAWDEV_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `CLAWDEV_AGENT_JWT_ISSUER` (default: `clawdev`)
  - `CLAWDEV_AGENT_JWT_AUDIENCE` (default: `clawdev-api`)
...
1. In:
   ...
   inject `CLAWDEV_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `CLAWDEV_API_KEY`, do not overwrite it.
- Continue injecting:
  - `CLAWDEV_AGENT_ID`
  - `CLAWDEV_COMPANY_ID`
  - `CLAWDEV_API_URL`
...
- Local adapters authenticate without manual `CLAWDEV_API_KEY` config.
