...
Today release smoke testing for published ClawDev packages is manual and shell-driven:
...
- runs `clawdev@${CLAWDEVAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@clawdev.local`
- password: `clawdev-smoke-password`
...
- the suite passes locally against both:
  - `CLAWDEVAI_VERSION=canary`
  - `CLAWDEVAI_VERSION=latest`
...
1. A published `clawdev@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `clawdev@latest` Docker install can be smoke-tested by Playwright in CI.
