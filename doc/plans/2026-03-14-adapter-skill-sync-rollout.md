...
This document defines the rollout plan for adapter-wide skill support in ClawDev.
...
- ClawDev stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

ClawDev currently has these adapters:
...
1. Desired skills live in ClawDev for every adapter.
...
These adapters have a stable local skills directory that ClawDev can read and manage.
...
These adapters do not have a meaningful ClawDev-owned persistent install state.
...
- desired skills stored in ClawDev
...
- runtime already injects ClawDev skills into `~/.cursor/skills`
...
- runtime already injects ClawDev skills into `~/.gemini/skills`
...
- runtime already injects ClawDev skills into `~/.pi/agent/skills`
...
- ClawDev can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects ClawDev skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- ClawDev may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed ClawDev skills that are clearly marked as ClawDev-managed
...
- ClawDev stores desired skills only
...
- avoid destructive wording unless ClawDev can prove a skill is ClawDev-managed
...
That gets ClawDev from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
