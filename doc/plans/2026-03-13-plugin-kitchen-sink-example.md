...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented ClawDev plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside ClawDev.
...
- package: `@clawdev/plugin-kitchen-sink-example`
- plugin id: `clawdev.kitchen-sink-example` or `clawdev-kitchen-sink-example`

Recommendation: use `clawdev-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core ClawDev event such as `issue.created`
...
Recommendation: default to a ClawDev-local endpoint or a stable public echo endpoint to avoid flaky docs.
