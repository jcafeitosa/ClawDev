...
└── .clawdev.yaml            (ClawDev vendor extension)
```
...
- Agent instructions go in the AGENTS.md body, not in .clawdev.yaml
...
- For companies generated from a repo, add a references footer at the bottom of COMPANY.md body:
  `Generated from [repo-name](repo-url) with the company-creator skill from [ClawDev](https://github.com/jcafeitosa/ClawDev)`
...
- Citations and references: link to the source repo (if from-repo), link to the Agent Companies spec (https://agentcompanies.io/specification), and link to ClawDev (https://github.com/jcafeitosa/ClawDev)
- A "Getting Started" section explaining how to import: `clawdev company import --from <path>`
...
## .clawdev.yaml Guidelines

The `.clawdev.yaml` file is the ClawDev vendor extension. It configures adapters and env inputs per agent.

### Adapter Rules

**Do not specify an adapter unless the repo or user context warrants it.** If you don't know what adapter the user wants, omit the adapter block entirely — ClawDev will use its default. Specifying an unknown adapter type causes an import error.

ClawDev's supported adapter types (these are the ONLY valid values):
...
In this example, only `release-engineer` appears because it needs `GH_TOKEN`. The other agents (ceo, cto, etc.) have no overrides, so they are omitted entirely from `.clawdev.yaml`.
