...
node packages/plugins/create-clawdev-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the ClawDev repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.clawdev-sdk/`:

```bash
pnpm --filter @clawdev/create-clawdev-plugin build
node packages/plugins/create-clawdev-plugin/dist/index.js @acme/plugin-name \
...
- is installed into ClawDev from an absolute local path during development
