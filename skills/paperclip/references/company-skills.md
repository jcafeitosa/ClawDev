...
```sh
curl -sS -X POST "$CLAWDEV_API_URL/api/companies/$CLAWDEV_COMPANY_ID/skills/import" \
...
```sh
curl -sS -X POST "$CLAWDEV_API_URL/api/companies/$CLAWDEV_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $CLAWDEV_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$CLAWDEV_API_URL/api/companies/$CLAWDEV_COMPANY_ID/skills" \
  -H "Authorization: Bearer $CLAWDEV_API_KEY"
```
...
```sh
curl -sS "$CLAWDEV_API_URL/api/companies/$CLAWDEV_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $CLAWDEV_API_KEY"

curl -sS "$CLAWDEV_API_URL/api/companies/$CLAWDEV_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $CLAWDEV_API_KEY"
```
...
```sh
curl -sS -X POST "$CLAWDEV_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $CLAWDEV_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$CLAWDEV_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $CLAWDEV_API_KEY"
```
...
```sh
curl -sS -X POST "$CLAWDEV_API_URL/api/companies/$CLAWDEV_COMPANY_ID/agents" \
  -H "Authorization: Bearer $CLAWDEV_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in ClawDev runtime skills are still added automatically when required by the adapter.
