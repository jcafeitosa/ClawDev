...
How to get OpenClaw running in a Docker container for local development and testing the ClawDev OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

ClawDev includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-clawdev-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a ClawDev host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-clawdev-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `CLAWDEV_HOST_PORT` (default `3100`)
- `CLAWDEV_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your ClawDev deployment is `authenticated`, provide auth context:

```bash
CLAWDEV_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
CLAWDEV_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host ClawDev server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed ClawDev URL (typically `http://host.docker.internal:3100`).
- If ClawDev rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm clawdev allowed-hostname host.docker.internal
```

Then restart ClawDev and rerun the smoke script.
