---
title: "ClipHub: Marketplace for ClawDev Team Configurations"
---
# ClipHub: Marketplace for ClawDev Team Configurations
...
> The "app store" for whole-company AI teams — pre-built ClawDev configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for ClawDev-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | ClawDev company (agents, projects, governance) |
...
A complete ClawDev company configuration:
...
Single-agent configurations designed to plug into a ClawDev org:
...
Portable skill files that any ClawDev agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with ClawDev's skill loading system
...
interface Listing {
  ...
  clawdevVersionMin: string;     // Minimum ClawDev version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target ClawDev company
...
| `POST` | `/api/listings/:id/install` | Install to ClawDev company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target ClawDev company (or create new)
  → ClipHub API calls ClawDev API to:
  ...
  → Redirect to ClawDev dashboard with new team running
```
...
### 5.3 Creator: Export from ClawDev → Publish

```
Running ClawDev company → "Export as Blueprint" (CLI or UI)
  → ClawDev exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (ClawDev brand blue/purple)
...
| Install flow | One-click deploy to ClawDev company |
...
{
  "targetCompanyId": "uuid",         // Existing ClawDev company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `clawdev-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as ClawDev
- **Backend**: Node.js API (or extend ClawDev server)
- **Database**: Postgres (can share ClawDev's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with ClawDev auth (or OAuth2)

### 9.2 Integration with ClawDev

ClipHub can be:
- **Option A**: A separate app that calls ClawDev's API to install blueprints
- **Option B**: A built-in section of the ClawDev UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing ClawDev UI and API.
...
- [ ] Install flow: blueprint → ClawDev company
...
- [ ] "Export from ClawDev" CLI command
