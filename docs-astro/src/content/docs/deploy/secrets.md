...
ClawDev encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.
...
```
~/.clawdev/instances/default/secrets/master.key
```
...
|----------|-------------|
| `CLAWDEV_SECRETS_MASTER_KEY` | 32-byte key as base64, hex, or raw string |
| `CLAWDEV_SECRETS_MASTER_KEY_FILE` | Custom key file path |
| `CLAWDEV_SECRETS_STRICT_MODE` | Set to `true` to enforce secret refs |
