...
ClawDev supports two runtime modes with different security profiles.
...
```sh
pnpm clawdev allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, ClawDev emits a one-time claim URL at startup:
...
```sh
CLAWDEV_DEPLOYMENT_MODE=authenticated pnpm clawdev run
```
