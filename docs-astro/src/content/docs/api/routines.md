...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-ClawDev-Signature` + `X-ClawDev-Timestamp` header pair matching the trigger's signing mode.
