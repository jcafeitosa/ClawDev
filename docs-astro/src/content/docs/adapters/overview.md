...
summary: What adapters are and how they connect agents to ClawDev
---

Adapters are the bridge between ClawDev's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, ClawDev:
...
| OpenClaw Gateway | `openclaw_gateway` | Connects to an OpenClaw gateway over WebSocket with gateway auth, device auth, and session routing |
...
packages/adapters/<name>/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `clawdev run --watch`
```
...
- **Need a coding agent?** Use `claude_local`, `codex_local`, `copilot_local`, `gemini_local`, `opencode_local`, or `pi_local`
- **Need to run a script or command?** Use `process`
- **Need to connect to OpenClaw natively?** Use `openclaw_gateway`
