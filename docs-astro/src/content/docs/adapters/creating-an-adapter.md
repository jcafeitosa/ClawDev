...
Build a custom adapter to connect ClawDev to any agent runtime.
...
2. Build environment with `buildClawDevEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `clawdev run --watch` using `picocolors`.
...
Make ClawDev skills discoverable to your agent runtime without writing to the agent's working directory:
