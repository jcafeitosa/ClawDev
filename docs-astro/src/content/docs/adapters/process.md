...
- Running a Python script that calls the ClawDev API
...
1. ClawDev spawns the configured command as a child process
2. Standard ClawDev environment variables are injected (`CLAWDEV_AGENT_ID`, `CLAWDEV_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the ClawDev API and perform work.
