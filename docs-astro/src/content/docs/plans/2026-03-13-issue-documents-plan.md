...
Add first-class **documents** to ClawDev as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by ClawDev workflow/docs.
...
Update the ClawDev agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/clawdev/SKILL.md`
