...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only ClawDev.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.clawdev.yaml`
...
- ClawDev must not require extra top-level fields for skill validity
- ClawDev-specific extensions must live under `metadata.clawdev` or `metadata.sources`
...
For ClawDev, the preferred fidelity extension is:
...
- issue/task ClawDev-only metadata
...
- ClawDev tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested ClawDev shape:
...
Additional rules for ClawDev exporters:
...
- omit empty and default-valued ClawDev fields when possible
...
- ClawDev exporters should emit `.clawdev.yaml` as a sidecar by default
...
ClawDev can map this spec to its runtime model like this:
...
- ClawDev extension:
  - `.clawdev.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other ClawDev-specific fidelity

Inline ClawDev-only metadata that must live inside a shared markdown file should use:

- `metadata.clawdev`

That keeps the base format broader than ClawDev.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only ClawDev.

## 20. Cutover

ClawDev should cut over to this markdown-first package model as the primary portability format.

`clawdev.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For ClawDev, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
