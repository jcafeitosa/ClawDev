#!/usr/bin/env bash
#
# Kill all local ClawDev dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all clawdev dev processes
...
# Collect PIDs of node processes running from any clawdev directory.
# Matches paths like /Users/*/clawdev/... or /Users/*/clawdev-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/clawdev(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No ClawDev dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} ClawDev dev process(es):"
echo ""
