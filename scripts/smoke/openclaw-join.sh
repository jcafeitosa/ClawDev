...
COMPANY_ID="${COMPANY_ID:-${CLAWDEV_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-clawdev-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-clawdev-openclaw-smoke}"
...
Provide one of:
  CLAWDEV_AUTH_HEADER=\"Bearer <board-token>\"
  CLAWDEV_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "ClawDev OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.clawdev.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
