...
CLAWDEV_HOST_PORT="${CLAWDEV_HOST_PORT:-3100}"
CLAWDEV_HOST_FROM_CONTAINER="${CLAWDEV_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.clawdev-openclaw.override.yml"
...
detect_clawdev_base_url() {
  ...
  for candidate in "$CLAWDEV_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${CLAWDEV_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${CLAWDEV_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
clawdev_base_url="$(detect_clawdev_base_url || true)"
...
ClawDev URL for OpenClaw container:
EOF
  if [[ -n "$clawdev_base_url" ]]; then
    cat <<EOF
  $clawdev_base_url
