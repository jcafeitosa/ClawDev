...
Environment overrides:
  ...
  PC_DATA=/tmp/data           ClawDev data dir to use
  CLAWDEV_HOST=127.0.0.1    Host passed to the onboarded server
  CLAWDEV_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/clawdev-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/clawdev-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export CLAWDEV_HOST="${CLAWDEV_HOST:-127.0.0.1}"
export CLAWDEV_PORT="${CLAWDEV_PORT:-3100}"
export CLAWDEV_OPEN_ON_LISTEN="${CLAWDEV_OPEN_ON_LISTEN:-false}"
...
echo "CLAWDEV_HOST: $CLAWDEV_HOST"
echo "CLAWDEV_PORT: $CLAWDEV_PORT"
