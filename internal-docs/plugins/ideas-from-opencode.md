...
A `@clawdev/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Paperclip instance.
...
- `@clawdev/plugin-test-harness` and `create-paperclip-plugin` starter template
...
8. Ship `@clawdev/plugin-test-harness` and `create-paperclip-plugin` starter template.
