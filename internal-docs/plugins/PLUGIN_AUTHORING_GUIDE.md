...
```bash
pnpm --filter @clawdev/create-paperclip-plugin build
...
Outside this monorepo, the scaffold snapshots `@clawdev/plugin-sdk` from the local Paperclip checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
