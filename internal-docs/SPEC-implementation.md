# ClawDev V1 Implementation Spec
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.clawdev/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.clawdev/instances/default/data/storage` (`local_disk`)
...
- `.clawdev.yaml` sidecar for ClawDev-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.clawdev.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and ClawDev routine fidelity in `.clawdev.yaml`
- ClawDev imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.clawdev.yaml`
