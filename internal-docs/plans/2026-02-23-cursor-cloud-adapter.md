...
This document defines the V1 design for a ClawDev adapter that integrates with
...
4. synthesized stdout events for ClawDev UI/CLI
...
3. **Agent callback auth to ClawDev** uses a bootstrap exchange flow (no long-lived ClawDev key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from ClawDev endpoints, not full SKILL.md prompt injection.
...
- `clawdevPublicUrl` (optional override; else `CLAWDEV_PUBLIC_URL` env)
...
## ClawDev Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `CLAWDEV_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.clawdevPublicUrl`
2. `process.env.CLAWDEV_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived ClawDev credentials in prompt text.

Flow:

1. Before launch/follow-up, ClawDev mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. ClawDev validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all ClawDev API calls.
...
1. Prompt includes a compact instruction to fetch skills from ClawDev.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/clawdev`
   - `GET /api/skills/clawdev-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <clawdevPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active ClawDev run context.
...
5. `clawdevPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from ClawDev skill endpoints |
| ClawDev auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from ClawDev context.
3. Add richer PR metadata surfacing in ClawDev UI.
