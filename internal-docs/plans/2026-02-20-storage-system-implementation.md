...
Add a single storage subsystem for ClawDev that supports:
...
- API serves attachment bytes through authenticated ClawDev endpoints.
...
- Default local path is under instance root: `~/.clawdev/instances/<instanceId>/data/storage`.
...
- `clawdev onboard` writes a valid `storage` config block by default.
- `clawdev configure --section storage` can switch between local and s3 modes.
...
- `clawdev doctor` reports actionable storage status.
