...
This document defines the product and UI plan for skill management in ClawDev.
...
- ClawDev company import/export can include skills as package content
...
3. The UI must always tell the truth about what ClawDev knows:
   - desired state in ClawDev
...
ClawDev should treat skills at two scopes:
...
- created locally inside ClawDev later
...
7. “Understand what ClawDev knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a ClawDev-specific package layout
...
This matters because Codex and similar adapters may already have local skills that ClawDev did not install.

These should be clearly marked:

- external
- not managed by ClawDev
...
- `desired_only`: ClawDev wants it, adapter does not show it yet
- `external`: adapter has it but ClawDev does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, ClawDev only tracks desired state
...
- ClawDev can still track desired skills
...
- ClawDev can see adapter skills
...
- `.clawdev.yaml` may add ClawDev-specific fidelity, but should not replace the base shortname association model
...
- `.clawdev.yaml` may add ClawDev-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
ClawDev should have a company-scoped skill model or managed package model representing:
...
ClawDev should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too ClawDev-specific.
...
That gives ClawDev one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
