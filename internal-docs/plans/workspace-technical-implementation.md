...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/clawdev-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `CLAWDEV_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `CLAWDEV_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - ClawDev may create host-local worktrees, processes, and paths
- `cloud`
  - ClawDev should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `clawdev | github | vercel | s3 | custom`
...
- use `CLAWDEV_EXECUTION_TOPOLOGY` as a defaulting guardrail
