...
This document defines the next-stage plan for ClawDev company import/export.

The core shift is:

- move from a ClawDev-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside ClawDev.
...
ClawDev already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just ClawDev
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `clawdev.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.clawdev.yaml` sidecar for high-fidelity ClawDev-specific details
...
11. adapter-aware skill sync surfaces so ClawDev can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point ClawDev at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a ClawDev sidecar.
...
ClawDev must not redefine `SKILL.md`.
...
- ClawDev-specific fidelity lives in `.clawdev.yaml`
- ClawDev may resolve and install `SKILL.md` packages, but it must not require a ClawDev-only skill format
...
### 5.4 Base Package Vs ClawDev Extension
...
- base package:
  ...
  - no ClawDev-only runtime fields by default
- ClawDev extension:
  - `.clawdev.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by ClawDev tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`clawdev.manifest.json` is not part of the future package direction.
...
ClawDev import/export should support these entity kinds:
...
- ClawDev can treat “this set of agents came from team package X” as the imported-team model
...
ClawDev should support source references in package metadata with:
...
ClawDev should define a new adapter surface area around skills:
...
Planned ClawDev behavior:

- if an adapter supports read, ClawDev should show current skills in the UI
- if an adapter supports write, ClawDev should let the user enable/disable imported skills
- if an adapter supports sync, ClawDev should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.clawdev.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.clawdev.yaml` alongside the base package
...
## 10. Storage Model Inside ClawDev
...
ClawDev should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `clawdev company export <company-id> --out <path>`
- `clawdev company import <path-or-url> --dry-run`
- `clawdev company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in ClawDev storage, or only referenced at import time?
...
3. Should ClawDev support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps ClawDev aligned with:
