...
ClawDev needs to support two very different but equally valid ways of working:
...
Today, ClawDev already has the beginnings of this model:
...
- making ClawDev feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing ClawDev users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted ClawDev deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning ClawDev into a full code review product
...
This object must be recorded explicitly so that ClawDev can:
...
ClawDev should treat PRs as a type of work product linked back to:
...
ClawDev already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted ClawDev must be a first-class deployment mode

ClawDev cannot assume that it is running on the same machine as the code.

In cloud deployments, ClawDev may:
...
- `Runtime service`: a process or service ClawDev owns or tracks for a workspace
...
It also lets cloud-hosted ClawDev deployments point at codebases and remotes without pretending the ClawDev host has direct filesystem access.
...
Without an explicit `execution workspace` record, ClawDev has nowhere stable to attach:
...
ClawDev needs a single place to show:
...
- ClawDev-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without ClawDev owning the execution host
...
## 1. ClawDev host and execution host must be decoupled
...
without the ClawDev host itself running local git or local preview processes.
...
ClawDev should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for ClawDev.
...
ClawDev should keep workspace automation centered on a small set of hooks:
...
ClawDev should provide the equivalent visibility in the project `Code` surface:
...
ClawDev should treat execution workspaces as the natural home for:
...
ClawDev should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
ClawDev should continue treating cleanup policy as part of the core workspace model:
...
ClawDev should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the ClawDev host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

ClawDev should borrow Worktrunk's good execution patterns while keeping the broader ClawDev model:
...
That keeps ClawDev focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted ClawDev deployments remain first-class

That is a better fit for ClawDev than either extreme:
