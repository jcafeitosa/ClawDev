...
ClipHub is the public registry where people share, discover, and download ClawDev company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to ClawDev what a package registry is to a programming language. ClawDev already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in ClawDev — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own ClawDev instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the ClawDev spec. It contains:
...
**Install (fresh start):**
```
clawdev install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local ClawDev instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within ClawDev, export your company as a template, then publish:

```
clawdev export --template my-company
clawdev publish cliphub my-company
```
...
ClipHub is a **separate service** from ClawDev itself. ClawDev is self-hosted; ClipHub is a hosted registry that ClawDev instances talk to.
...
| **ClawDev CLI** | `clawdev install`, `clawdev publish`, `clawdev cliphub sync` — built into ClawDev |
| **ClawDev UI** | "Browse ClipHub" panel in the ClawDev web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with ClawDev) |
| Backend | TypeScript + Hono (consistent with ClawDev) |
| Database | PostgreSQL |
...
4. Run `clawdev install cliphub:acme/lean-saas-shop`
5. ClawDev creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in ClawDev until it works well
2. Export: `clawdev export --template my-agency`
3. Publish: `clawdev publish cliphub my-agency`
...
3. Install just that agent: `clawdev install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to ClawDev

ClipHub is **not required** to use ClawDev. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to ClawDev what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `clawdev install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in ClawDev web UI
- [ ] `clawdev cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with ClawDev's own philosophy)
