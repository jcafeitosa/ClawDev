...
const JWT_SECRET_ENV_KEY = "CLAWDEV_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by ClawDev CLI commands",
...
export function resolveClawDevEnvFile(configPath?: string): string {
...
export function loadClawDevEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeClawDevEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readClawDevEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeClawDevEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeClawDevEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readClawDevEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeClawDevEnvEntries(next, filePath);
  return next;
}
