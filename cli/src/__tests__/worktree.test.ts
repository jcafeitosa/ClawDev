...
function buildSourceConfig(): ClawDevConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("clawdev-pr-432")).toBe(
      path.resolve(os.homedir(), "clawdev-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("clawdev/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://clawdev.example", 3110)).toBe("https://clawdev.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/clawdev-feature",
      homeDir: "/tmp/clawdev-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/clawdev-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/clawdev-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.CLAWDEV_HOME).toBe(path.resolve("/tmp/clawdev-worktrees"));
    expect(env.CLAWDEV_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.CLAWDEV_IN_WORKTREE).toBe("true");
    expect(env.CLAWDEV_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.CLAWDEV_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export CLAWDEV_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    ...
    try {
      delete process.env.CLAWDEV_SECRETS_MASTER_KEY;
      delete process.env.CLAWDEV_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.CLAWDEV_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.CLAWDEV_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.CLAWDEV_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          CLAWDEV_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "clawdev-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.CLAWDEV_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.CLAWDEV_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".clawdev-worktrees"),
      });

      const envPath = path.join(repoRoot, ".clawdev", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("CLAWDEV_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("CLAWDEV_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/CLAWDEV_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.CLAWDEV_AGENT_JWT_SECRET;
      } else {
        process.env.CLAWDEV_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "clawdev-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".clawdev-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".clawdev", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local ClawDev config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "clawdev-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".clawdev", "config.json");
    const originalCwd = process.cwd();
    const originalClawDevConfig = process.env.CLAWDEV_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalClawDevConfig === undefined) {
        delete process.env.CLAWDEV_CONFIG;
      } else {
        process.env.CLAWDEV_CONFIG = originalClawDevConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "clawdev-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/clawdev",
        targetRepoRoot: "/Users/example/clawdev-pr-432",
        workspaceCwd: "/Users/example/clawdev",
      }),
    ).toBe("/Users/example/clawdev-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/clawdev",
        targetRepoRoot: "/Users/example/clawdev-pr-432",
        workspaceCwd: "/Users/example/clawdev/packages/db",
      }),
    ).toBe("/Users/example/clawdev-pr-432/packages/db");
  });
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "clawdev-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "clawdev-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("clawdev-make-test", {
        seed: false,
        home: path.join(tempRoot, ".clawdev-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".clawdev", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".clawdev", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
