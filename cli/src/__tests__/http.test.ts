...
import { ApiConnectionError, ApiRequestError, ClawDevApiClient } from "../client/http.js";

describe("ClawDevApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new ClawDevApiClient({
    ...
    expect(headers["x-clawdev-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new ClawDevApiClient({ apiBase: "http://localhost:3100" });
