...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@clawdev/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://clawdev:clawdev@127.0.0.1:${port}/clawdev`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" clawdev db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveClawDevInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "clawdev";
