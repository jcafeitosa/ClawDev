...
function buildAgentEnvExports(input: {
  ...
  return [
    `export CLAWDEV_API_URL='${escaped(input.apiBase)}'`,
    `export CLAWDEV_COMPANY_ID='${escaped(input.companyId)}'`,
    `export CLAWDEV_AGENT_ID='${escaped(input.agentId)}'`,
    `export CLAWDEV_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local ClawDev skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing ClawDev skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveClawDevSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local ClawDev skills directory. Expected ./skills in the repo checkout.",
              );
            }
