...
import { ApiRequestError, ClawDevApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to ClawDev config file")
    .option("-d, --data-dir <path>", "ClawDev data directory root (isolates state from ~/.clawdev)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the ClawDev API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.CLAWDEV_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.CLAWDEV_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.CLAWDEV_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set CLAWDEV_COMPANY_ID, or set context profile companyId via `clawdev context set`.",
    );
  }

  const api = new ClawDevApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.CLAWDEV_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.CLAWDEV_SERVER_PORT || "");
