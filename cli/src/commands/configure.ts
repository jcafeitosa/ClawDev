...
function defaultConfig(): ClawDevConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" clawdev configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `clawdev onboard` first.");
    p.outro("");
    return;
  }

  let config: ClawDevConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because CLAWDEV_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
