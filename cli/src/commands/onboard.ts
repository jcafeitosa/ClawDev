...
type OnboardDefaults = Pick<ClawDevConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "CLAWDEV_PUBLIC_URL",
  "DATABASE_URL",
  "CLAWDEV_DB_BACKUP_ENABLED",
  "CLAWDEV_DB_BACKUP_INTERVAL_MINUTES",
  "CLAWDEV_DB_BACKUP_RETENTION_DAYS",
  "CLAWDEV_DB_BACKUP_DIR",
  "CLAWDEV_DEPLOYMENT_MODE",
  "CLAWDEV_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "CLAWDEV_ALLOWED_HOSTNAMES",
  "CLAWDEV_AUTH_BASE_URL_MODE",
  "CLAWDEV_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "CLAWDEV_STORAGE_PROVIDER",
  "CLAWDEV_STORAGE_LOCAL_DIR",
  "CLAWDEV_STORAGE_S3_BUCKET",
  "CLAWDEV_STORAGE_S3_REGION",
  "CLAWDEV_STORAGE_S3_ENDPOINT",
  "CLAWDEV_STORAGE_S3_PREFIX",
  "CLAWDEV_STORAGE_S3_FORCE_PATH_STYLE",
  "CLAWDEV_SECRETS_PROVIDER",
  "CLAWDEV_SECRETS_STRICT_MODE",
  "CLAWDEV_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const publicUrl =
    process.env.CLAWDEV_PUBLIC_URL?.trim() ||
    process.env.CLAWDEV_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.CLAWDEV_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.CLAWDEV_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.CLAWDEV_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.CLAWDEV_ALLOWED_HOSTNAMES
    ? process.env.CLAWDEV_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.CLAWDEV_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.CLAWDEV_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.CLAWDEV_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.CLAWDEV_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.CLAWDEV_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.CLAWDEV_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.CLAWDEV_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.CLAWDEV_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.CLAWDEV_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.CLAWDEV_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.CLAWDEV_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.CLAWDEV_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.CLAWDEV_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.CLAWDEV_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.CLAWDEV_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "CLAWDEV_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<ClawDevConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printClawDevCliBanner();
  p.intro(pc.bgCyan(pc.black(" clawdev onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveClawDevInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: ClawDevConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@clawdev/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `clawdev doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("CLAWDEV_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.CLAWDEV_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("CLAWDEV_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("CLAWDEV_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: ClawDevConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: CLAWDEV_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("clawdev run")}`,
      `Reconfigure later: ${pc.cyan("clawdev configure")}`,
      `Diagnose setup: ${pc.cyan("clawdev doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.CLAWDEV_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("clawdev run")}`,
        `Then: ${pc.cyan("clawdev auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
