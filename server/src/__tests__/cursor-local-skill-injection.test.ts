...
import { ensureCursorSkillsInjected } from "@clawdev/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing ClawDev skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("clawdev-cursor-skills-src-");
    const skillsHome = await makeTempDir("clawdev-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "clawdev");
    await createSkillDir(skillsDir, "clawdev-create-agent");
    ...
    const injectedA = path.join(skillsHome, "clawdev");
    const injectedB = path.join(skillsHome, "clawdev-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "clawdev")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "clawdev-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "clawdev"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "clawdev-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("clawdev-cursor-preserve-src-");
    const skillsHome = await makeTempDir("clawdev-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "clawdev");
    ...
    expect((await fs.lstat(path.join(skillsHome, "clawdev-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("clawdev-cursor-fail-src-");
    const skillsHome = await makeTempDir("clawdev-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
