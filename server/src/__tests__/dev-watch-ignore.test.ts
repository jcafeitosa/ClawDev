...
describe("resolveServerDevWatchIgnorePaths", () => {
  it("includes both the worktree UI paths and their real shared targets", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "clawdev-dev-watch-"));
    const sharedUiRoot = path.join(tempRoot, "shared-ui");
    const worktreeRoot = path.join(tempRoot, "repo", ".clawdev", "worktrees", "PAP-884");
    const serverRoot = path.join(worktreeRoot, "server");
    const worktreeUiRoot = path.join(worktreeRoot, "svelte-ui");

    fs.mkdirSync(path.join(sharedUiRoot, "node_modules"), { recursive: true });
    fs.mkdirSync(path.join(sharedUiRoot, ".vite"), { recursive: true });
    fs.mkdirSync(path.join(sharedUiRoot, "build"), { recursive: true });
    ...
    fs.symlinkSync(path.join(sharedUiRoot, "build"), path.join(worktreeUiRoot, "build"));
    ...
    expect(ignorePaths).toContain(path.join(worktreeUiRoot, "build"));
    expect(ignorePaths).toContain(fs.realpathSync(path.join(sharedUiRoot, "build")));
