...
describe("routine routes", () => {
  ...
  it("requires tasks:assign permission to create a trigger", async () => {
    ...
    const res = await req(app, "POST", `/api/routines/${routineId}/triggers`, {
      kind: "schedule",
      cronExpression: "0 10 * * *",
      timezone: "UTC",
    });
  ...
  it("requires tasks:assign permission to update a trigger", async () => {
    ...
    const res = await req(app, "PATCH", `/api/routine-triggers/${trigger.id}`, {
      enabled: true,
    });
  ...
  it("requires tasks:assign permission to manually run a routine", async () => {
    ...
    const res = await req(app, "POST", `/api/routines/${routineId}/run`, {});
