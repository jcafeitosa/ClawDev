...
<head>
    <!-- CLAWDEV_RUNTIME_BRANDING_START -->
    <!-- CLAWDEV_RUNTIME_BRANDING_END -->
    <!-- CLAWDEV_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from CLAWDEV_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ CLAWDEV_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ CLAWDEV_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ CLAWDEV_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      CLAWDEV_IN_WORKTREE: "true",
      CLAWDEV_WORKTREE_NAME: "clawdev-pr-432",
      CLAWDEV_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("clawdev-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="clawdev-worktree-name"');
    expect(meta).toContain('content="clawdev-pr-432"');
    expect(meta).toContain('name="clawdev-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="clawdev-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="clawdev-worktree-name"');
  });
});
