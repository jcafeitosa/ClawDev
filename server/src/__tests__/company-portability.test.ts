...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "clawdev",
        name: "clawdev",
        description: "ClawDev coordination skill",
        markdown: "---\nname: clawdev\ndescription: ClawDev coordination skill\n---\n\n# ClawDev\n",
        sourceType: "github",
        sourceLocator: "https://github.com/jcafeitosa/ClawDev/tree/master/skills/clawdev",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/jcafeitosa/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "clawdev",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/jcafeitosa/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "clawdev",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized ClawDev extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "ClawDev"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${clawdevKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/jcafeitosa/ClawDev/clawdev/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/jcafeitosa/ClawDev/clawdev/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/jcafeitosa/ClawDev/clawdev/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".clawdev.yaml"]);
    expect(extension).toContain('schema: "clawdev/v1"');
    ...
    expect(extension).not.toContain("clawdevSkillSync");
  ...
  it("exports default sidebar order into the ClawDev extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".clawdev.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/jcafeitosa/ClawDev/clawdev/SKILL.md"])).toContain("# ClawDev");
    expect(asTextFile(exported.files["skills/jcafeitosa/ClawDev/clawdev/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/jcafeitosa/ClawDev/clawdev/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/jcafeitosa/ClawDev/clawdev/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/jcafeitosa/ClawDev/clawdev/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .clawdev.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".clawdev.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-clawdev",
        companyId: "company-1",
        key: "jcafeitosa/ClawDev/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/jcafeitosa/ClawDev/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/jcafeitosa/ClawDev/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/jcafeitosa/ClawDev/release-changelog/SKILL.md"])).toContain("jcafeitosa/ClawDev/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/clawdev",
            repoUrl: "https://github.com/jcafeitosa/ClawDev.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .clawdev-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/jcafeitosa/ClawDev.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/clawdev");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported ClawDev",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdev-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/jcafeitosa/ClawDev.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".clawdev.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/jcafeitosa/ClawDev.git"');
    expect(extension).toContain('projectWorkspaceKey: "clawdev"');
  ...
  it("reads env inputs back from .clawdev.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with ClawDev routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "clawdev-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported ClawDev" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported ClawDev"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .clawdev.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported ClawDev");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".clawdev.yaml"] = `${exported.files[".clawdev.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
