...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        markdown: "---\nname: clawdev\ndescription: ClawDev coordination skill\n---\n\n# ClawDev\n",
        sourceType: "github",
        sourceLocator: "https://github.com/clawdev/clawdev/tree/master/skills/clawdev",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/clawdev/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "clawdev",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/clawdev/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "clawdev",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized ClawDev extension data", async () => {
    ...
    expect(asTextFile(exported.files["skills/clawdev/clawdev/clawdev/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/clawdev/clawdev/clawdev/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/clawdev/clawdev/clawdev/references/api.md"]).toBeUndefined();
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/clawdev/clawdev/clawdev/SKILL.md"])).toContain("# ClawDev");
    expect(asTextFile(exported.files["skills/clawdev/clawdev/clawdev/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/clawdev/clawdev/clawdev/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/clawdev/clawdev/clawdev/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/clawdev/clawdev/clawdev/SKILL.md"]).toBeDefined();
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-clawdev",
        companyId: "company-1",
        key: "clawdev/clawdev/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/clawdev/clawdev/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/clawdev/clawdev/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/clawdev/clawdev/release-changelog/SKILL.md"])).toContain("clawdev/clawdev/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            repoUrl: "https://github.com/clawdev/clawdev.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
    ...
    expect(extension).toContain('repoUrl: "https://github.com/clawdev/clawdev.git"');
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    ...
    execFileSync("git", ["remote", "add", "origin", "https://github.com/clawdev/clawdev.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
