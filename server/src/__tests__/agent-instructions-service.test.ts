...
describe("agent instructions service", () => {
  const originalClawDevHome = process.env.CLAWDEV_HOME;
  const originalClawDevInstanceId = process.env.CLAWDEV_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalClawDevHome === undefined) delete process.env.CLAWDEV_HOME;
    else process.env.CLAWDEV_HOME = originalClawDevHome;
    if (originalClawDevInstanceId === undefined) delete process.env.CLAWDEV_INSTANCE_ID;
    else process.env.CLAWDEV_INSTANCE_ID = originalClawDevInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const clawdevHome = await makeTempDir("clawdev-agent-instructions-home-");
    const externalRoot = await makeTempDir("clawdev-agent-instructions-external-");
    cleanupDirs.add(clawdevHome);
    cleanupDirs.add(externalRoot);
    process.env.CLAWDEV_HOME = clawdevHome;
    process.env.CLAWDEV_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("clawdev-agent-instructions-new-external-");
    cleanupDirs.add(clawdevHome);
    cleanupDirs.add(externalRoot);
    process.env.CLAWDEV_HOME = clawdevHome;
    process.env.CLAWDEV_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("clawdev-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const clawdevHome = await makeTempDir("clawdev-agent-instructions-recover-");
    cleanupDirs.add(clawdevHome);
    process.env.CLAWDEV_HOME = clawdevHome;
    process.env.CLAWDEV_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const clawdevHome = await makeTempDir("clawdev-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("clawdev-agent-instructions-stale-root-");
    cleanupDirs.add(clawdevHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const clawdevHome = await makeTempDir("clawdev-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("clawdev-agent-instructions-heal-write-stale-");
    cleanupDirs.add(clawdevHome);
    cleanupDirs.add(staleRoot);
    process.env.CLAWDEV_HOME = clawdevHome;
    process.env.CLAWDEV_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const clawdevHome = await makeTempDir("clawdev-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("clawdev-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(clawdevHome);
    cleanupDirs.add(staleRoot);
    process.env.CLAWDEV_HOME = clawdevHome;
    process.env.CLAWDEV_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const clawdevHome = await makeTempDir("clawdev-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("clawdev-agent-instructions-partial-root-");
    cleanupDirs.add(clawdevHome);
    cleanupDirs.add(staleRoot);
    process.env.CLAWDEV_HOME = clawdevHome;
    process.env.CLAWDEV_INSTANCE_ID = "test-instance";
