...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".clawdev.yaml" || normalized === ".clawdev.yml") return "extension";
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "clawdev_bundled") {
    return `jcafeitosa/ClawDev/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .clawdev.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .clawdev.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .clawdev.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .clawdev.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; ClawDev will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .clawdev.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .clawdev.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .clawdev.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .clawdev.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .clawdev.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .clawdev.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .clawdev.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .clawdev.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .clawdev.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[clawdevExtensionPath];
  if (selectedFiles.has(clawdevExtensionPath) && typeof extensionEntry === "string") {
    filtered[clawdevExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findClawDevExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".clawdev.yaml"] === "string") return ".clawdev.yaml";
  if (typeof files[".clawdev.yml"] === "string") return ".clawdev.yml";
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "clawdev_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "jcafeitosa/ClawDev",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/jcafeitosa/ClawDev/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "clawdev_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.clawdevSkillKey = skill.key;
  metadata.clawdev = {
    ...(isPlainRecord(metadata.clawdev) ? metadata.clawdev : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const clawdevExtensionPath = findClawDevExtensionPath(normalizedFiles);
  const clawdevExtension = clawdevExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, clawdevExtensionPath) ?? "")
    : {};
  const clawdevCompany = isPlainRecord(clawdevExtension.company) ? clawdevExtension.company : {};
  const clawdevSidebar = normalizePortableSidebarOrder(clawdevExtension.sidebar);
  const clawdevAgents = isPlainRecord(clawdevExtension.agents) ? clawdevExtension.agents : {};
  const clawdevProjects = isPlainRecord(clawdevExtension.projects) ? clawdevExtension.projects : {};
  const clawdevTasks = isPlainRecord(clawdevExtension.tasks) ? clawdevExtension.tasks : {};
  const clawdevRoutines = isPlainRecord(clawdevExtension.routines) ? clawdevExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(clawdevCompany.brandColor),
      logoPath: asString(clawdevCompany.logoPath) ?? asString(clawdevCompany.logo),
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(clawdevAgents[slug]) ? clawdevAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(clawdevProjects[slug]) ? clawdevProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(clawdevTasks[slug]) ? clawdevTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(clawdevRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(clawdevRoutines[slug]) ? clawdevRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".clawdev.yaml" ||
          relative === ".clawdev.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const clawdevAgentsOut: Record<string, Record<string, unknown>> = {};
    const clawdevProjectsOut: Record<string, Record<string, unknown>> = {};
    const clawdevTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const clawdevRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readClawDevSkillSyncPreference(
        ...
        clawdevAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      clawdevProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      clawdevTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      clawdevRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const clawdevExtensionPath = ".clawdev.yaml";
    const clawdevAgents = Object.fromEntries(
      Object.entries(clawdevAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const clawdevProjects = Object.fromEntries(
      Object.entries(clawdevProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const clawdevTasks = Object.fromEntries(
      Object.entries(clawdevTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const clawdevRoutines = Object.fromEntries(
      Object.entries(clawdevRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[clawdevExtensionPath] = buildYamlFile(
      {
        schema: "clawdev/v1",
        ...
        agents: Object.keys(clawdevAgents).length > 0 ? clawdevAgents : undefined,
        projects: Object.keys(clawdevProjects).length > 0 ? clawdevProjects : undefined,
        tasks: Object.keys(clawdevTasks).length > 0 ? clawdevTasks : undefined,
        routines: Object.keys(clawdevRoutines).length > 0 ? clawdevRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, clawdevExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeClawDevSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
