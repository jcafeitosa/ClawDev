 ...
 */
import { pluginManifestV1Schema } from "@clawdev/shared";
import type { ClawDevPluginManifestV1 } from "@clawdev/shared";
import { PLUGIN_API_VERSION } from "@clawdev/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): ClawDevPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as ClawDevPluginManifestV1,
        };
      }
