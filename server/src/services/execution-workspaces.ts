...
import type { ExecutionWorkspace, ExecutionWorkspaceConfig } from "@clawdev/shared";
import { parseProjectExecutionWorkspacePolicy } from "./execution-workspace-policy.js";

type ExecutionWorkspaceRow = typeof executionWorkspaces.$inferSelect;
type WorkspaceRuntimeServiceRow = typeof workspaceRuntimeServices.$inferSelect;
type ExecutionWorkspaceConfigLike = {
...
function readExecutionWorkspaceConfig(metadata: Record<string, unknown> | null | undefined): ExecutionWorkspaceConfigLike | null {
  ...
  const config: ExecutionWorkspaceConfigLike = {
    ...
    workspaceRuntime: isRecord(raw.workspaceRuntime) ? { ...raw.workspaceRuntime } : null,
    desiredState: raw.desiredState === "running" || raw.desiredState === "stopped" ? raw.desiredState : null,
  };

  const hasConfig =
    config.provisionCommand !== null ||
    config.cleanupCommand !== null ||
    config.teardownCommand !== null ||
    config.desiredState !== null ||
    (config.workspaceRuntime ? Object.keys(config.workspaceRuntime).length > 0 : false);
...
export function mergeExecutionWorkspaceConfig(
  ...
  const nextConfig: ExecutionWorkspaceConfigLike = {
    provisionCommand:
    ...
    workspaceRuntime:
      patch.workspaceRuntime !== undefined ? (isRecord(patch.workspaceRuntime) ? { ...patch.workspaceRuntime } : null) : current.workspaceRuntime,
    desiredState:
  ...
  const hasConfig =
    nextConfig.provisionCommand !== null ||
    nextConfig.teardownCommand !== null ||
    nextConfig.cleanupCommand !== null ||
    nextConfig.desiredState !== null ||
    (nextConfig.workspaceRuntime ? Object.keys(nextConfig.workspaceRuntime).length > 0 : false);

  if (!hasConfig) {
    delete nextMetadata.config;
  } else {
    nextMetadata.config = nextConfig;
  }

  return Object.keys(nextMetadata).length > 0 ? nextMetadata : null;
}

function toRuntimeService(row: WorkspaceRuntimeServiceRow) {
  return {
    id: row.id,
    ...
    scopeType: row.scopeType as "project_workspace" | "execution_workspace" | "run" | "agent",
    scopeId: row.scopeId ?? null,
    serviceName: row.serviceName,
    status: row.status as "starting" | "running" | "stopped" | "failed",
    lifecycle: row.lifecycle as "shared" | "ephemeral",
    reuseKey: row.reuseKey ?? null,
    command: row.command ?? null,
    cwd: row.cwd ?? null,
    port: row.port ?? null,
    url: row.url ?? null,
    provider: row.provider as "local_process" | "adapter_managed",
    ...
    healthStatus: row.healthStatus as "unknown" | "healthy" | "unhealthy",
...
async function inspectGitCloseReadiness(workspace: ExecutionWorkspace): Promise<{
  ...
  if (!workspacePath) {
    warnings.push("Workspace has no local path, so ClawDev cannot inspect git status before close.");
    return { git: null, warnings };
  }

  if (!(await pathExists(workspacePath))) {
    warnings.push(`Workspace path "${workspacePath}" does not exist, so ClawDev cannot inspect git status before close.`);
    return {
      git: {
        repoRoot: null,
        workspacePath,
...
export function executionWorkspaceService(db: Db) {
  return {
    ...
    getCloseReadiness: async (id: string) => {
      ...
      const workspacePath = git?.workspacePath ?? readNullableString(executionWorkspace.providerRef) ?? readNullableString(executionWorkspace.cwd);
      ...
      const plannedActions: Array<{ kind: string; label: string; description: string; command: string | null }> = [
        {
          kind: "archive_record",
      ...
      if (executionWorkspace.providerType === "git_worktree" && workspacePath) {
        plannedActions.push({
          ...
          description: `ClawDev will run git worktree cleanup for ${workspacePath}.`,
      ...
      if (git?.createdByRuntime && executionWorkspace.branchName) {
        plannedActions.push({
          ...
          description: "ClawDev will try to delete the runtime-created branch after removing the worktree.",
      ...
      if (executionWorkspace.providerType === "local_fs" && git?.createdByRuntime && workspacePath) {
        ...
        if (containsProjectWorkspace) {
          warnings.push(`ClawDev will archive this workspace but keep "${workspacePath}" because it contains the project workspace.`);
        } else {
          plannedActions.push({
            kind: "remove_local_directory",
            label: "Remove runtime-created directory",
            description: `ClawDev will remove the runtime-created directory at ${workspacePath}.`,
