...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@clawdev/db";
 ...
 *
 * This implementation delegates to the core ClawDev domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@clawdev/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
