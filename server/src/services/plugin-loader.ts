 ...
 *    (`~/.clawdev/plugins/`) and `node_modules` for packages matching
 *    the `clawdev-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published ClawDev plugins.
 * Packages matching this pattern are considered ClawDev plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "clawdev-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: ClawDevPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.clawdev/plugins/ local directory
  | "npm"               // npm packages matching clawdev-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: ClawDevPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the clawdev-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "clawdev-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: ClawDevPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover ClawDev plugins installed as npm packages in the current
   * Node.js environment matching the "clawdev-plugin-*" naming convention.
   *
   ...
   * the "clawdevPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a ClawDev plugin.
   * Throws if the package is a ClawDev plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<ClawDevPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: ClawDevPluginManifestV1;
    newManifest: ClawDevPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the ClawDev plugin naming convention.
 * Accepts both the "clawdev-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @clawdev/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "clawdevPlugin" key in package.json with a "manifest"
...
async function resolveManifestPath(
  ...
  if (
    clawdevPlugin !== null &&
    typeof clawdevPlugin === "object" &&
    !Array.isArray(clawdevPlugin)
  ) {
    const manifestRelPath = (clawdevPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: ClawDevPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumClawDevVersion;
}
 ...
 *   packageName: "clawdev-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: ClawDevPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as ClawDevPluginManifestV1 | null;
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<ClawDevPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a ClawDev plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasClawDevPlugin = "clawdevPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasClawDevPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<ClawDevPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: ClawDevPluginManifestV1;
      } | null;
