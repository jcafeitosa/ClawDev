...
/** Resolve the ClawDev repo skills directory (built-in / managed skills). */
function resolveClawDevSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  // Build set of ClawDev-managed skill names
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.clawdevApiUrl)) {
    const legacyClawDevApiUrl = nonEmptyTrimmedString(input.clawdevApiUrl);
    if (legacyClawDevApiUrl) merged.clawdevApiUrl = legacyClawDevApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    clawdevApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.clawdevApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawClawDevApiUrl =
    typeof defaults.clawdevApiUrl === "string"
      ? defaults.clawdevApiUrl.trim()
      : "";
  if (rawClawDevApiUrl) {
    try {
      const parsedClawDevApiUrl = new URL(rawClawDevApiUrl);
      if (
        parsedClawDevApiUrl.protocol !== "http:" &&
        parsedClawDevApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_clawdev_api_url_protocol",
          level: "warn",
          message: `clawdevApiUrl must use http:// or https:// (got ${parsedClawDevApiUrl.protocol}).`
        });
      } else {
        normalized.clawdevApiUrl = parsedClawDevApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_clawdev_api_url_configured",
          level: "info",
          message: `clawdevApiUrl set to ${parsedClawDevApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_clawdev_api_url_invalid",
        level: "warn",
        message: `Invalid clawdevApiUrl: ${rawClawDevApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your ClawDev host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "ClawDev is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm clawdev allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/clawdev";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/clawdev-claimed-api-key.json and load CLAWDEV_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: clawdevApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the ClawDev hostname is reachable and allowed via `pnpm clawdev allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this ClawDev API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, ClawDev generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "clawdevApiUrl": "https://clawdev-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in ClawDev before key claim is allowed.
    ...
    - ~/.openclaw/workspace/clawdev-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/clawdev-claimed-api-key.json

    And set the CLAWDEV_API_KEY and CLAWDEV_API_URL in your environment variables as specified here:
    ...
    Then set CLAWDEV_API_KEY and CLAWDEV_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install ClawDev skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your CLAWDEV_API_URL to the top of your skill and note the path to your CLAWDEV_API_URL
    ...
    ${
      ...
      "Ensure ClawDev is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested ClawDev base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.clawdevApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm clawdev allowed-hostname <host>
      - then restart ClawDev and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@clawdev.local";
    ...
    res.json({
      skills: [
        ...
        },
        {
          ...
          path: "/api/skills/clawdev-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              clawdevApiUrl: req.body.clawdevApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.clawdevApiUrl &&
          !persistedDefaults.clawdevApiUrl
        ) {
          missingPersistedFields.push("clawdevApiUrl");
        }
