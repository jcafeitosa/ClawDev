/**
 * Core types for the ClawDev plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@clawdev/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @clawdev/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of ClawDev object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the ClawDev UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit ClawDev domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core ClawDev domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the ClawDev secret provider
...
export interface PluginSecretsClient {
   ...
   * in the ClawDev secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@clawdev/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@clawdev/plugin-sdk";
 *
