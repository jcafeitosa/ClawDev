 ...
 * channel.
 *
 * The ClawDev plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@clawdev/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all ClawDev messages.
 */
...
/**
 * ClawDev plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this ClawDev instance. */
    instanceId: string;
    /** Semver version of the running ClawDev host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
