...
Official TypeScript SDK for ClawDev plugin authors.

- **Worker SDK:** `@clawdev/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@clawdev/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@clawdev/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@clawdev/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@clawdev/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@clawdev/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@clawdev/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@clawdev/plugin-sdk/ui/hooks` | Hooks only |
| `@clawdev/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@clawdev/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@clawdev/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@clawdev/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@clawdev/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@clawdev/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main ClawDev app. They can call ordinary ClawDev HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the ClawDev instance at runtime.
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@clawdev/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core ClawDev information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@clawdev/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@clawdev/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@clawdev/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@clawdev/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@clawdev/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@clawdev/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@clawdev/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@clawdev/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@clawdev/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@clawdev/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@clawdev/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@clawdev/plugin-sdk/bundlers";
...
```bash
clawdev-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@clawdev/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__clawdev__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__clawdev__/events` streams `reload` SSE events on UI build changes
