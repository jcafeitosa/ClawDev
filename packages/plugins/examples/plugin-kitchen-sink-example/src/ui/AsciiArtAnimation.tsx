...
type ClawDevSprite = (typeof CLAWDEV_SPRITES)[number];
...
function spriteSize(sprite: ClawDevSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = CLAWDEV_SPRITES[(baseCol + baseRow) % CLAWDEV_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = CLAWDEV_SPRITES[Math.floor(Math.random() * CLAWDEV_SPRITES.length)]!;
