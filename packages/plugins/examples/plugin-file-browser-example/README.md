...
Example ClawDev plugin that demonstrates:
...
```bash
pnpm --filter @clawdev/plugin-file-browser-example build
pnpm clawdev plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm clawdev plugin uninstall clawdev-file-browser-example --force
```
...
- Optional: use `clawdev-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
