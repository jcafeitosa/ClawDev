...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".clawdev-sdk");
...
/**
 * Generate a complete ClawDev plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A ClawDev plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "clawdev-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["clawdev", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@clawdev/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
...
`,
  ...
  writeFile(
    ...
    `import type { ClawDevPluginManifestV1 } from "@clawdev/plugin-sdk";

const manifest: ClawDevPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@clawdev/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@clawdev/plugin-sdk\` and \`@clawdev/shared\` from a local ClawDev checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.clawdev-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-clawdev-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <clawdev-sdk-path>]");
    process.exit(1);
  }
