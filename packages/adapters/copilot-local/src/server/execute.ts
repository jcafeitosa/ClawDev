...
async function buildCopilotRuntimeConfig(input: CopilotExecutionInput): Promise<CopilotRuntimeConfig> {
  ...
  if (runtimeServiceIntents.length > 0) env.CLAWDEV_RUNTIME_SERVICE_INTENTS_JSON = JSON.stringify(runtimeServiceIntents);
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const commandNotes = instructionsFilePath
    ? [`Injected agent instructions via system prompt from ${instructionsFilePath}`]
    : [];
  ...
  // Read instructions file content if configured, to prepend to prompt
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    // Copilot CLI does not support stdin piping — prompt goes via -p flag
    ...
    const parsedStream = parseCopilotJsonOutput(proc.stdout);
  ...
  const toAdapterResult = (
    attempt: {
      ...
      parsedStream: ReturnType<typeof parseCopilotJsonOutput>;
    ...
    const resolvedSessionId =
      ...
      (asString(parsed.sessionId ?? parsed.session_id, opts.fallbackSessionId ?? "") || opts.fallbackSessionId);
    const resolvedSessionParams = resolvedSessionId
      ...
      : null;
    const clearSessionForMaxTurns = isCopilotMaxTurnsResult(parsed);
    ...
    return {
      ...
      errorMessage:
        (proc.exitCode ?? 0) === 0
          ? null
          : describeCopilotFailure(parsed) ?? `Copilot exited with code ${proc.exitCode ?? -1}`,
      errorCode: loginMeta.requiresLogin ? "copilot_auth_required" : null,
      errorMeta,
      usage,
      sessionId: resolvedSessionId,
      ...
      provider: "github",
      biller: "github",
      model: parsedStream.model || asString(parsed.model, model || "gpt-4.1"),
      billingType: "subscription",
      costUsd: parsedStream.costUsd ?? asNumber(parsed.total_cost_usd ?? parsed.costUsd, 0),
      resultJson: parsed,
  ...
  try {
    ...
    if (
      ...
      initial.parsed &&
      isCopilotUnknownSessionError(initial.parsed)
    ) {
      await onLog(
        "stdout",
        `[clawdev] Copilot resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
