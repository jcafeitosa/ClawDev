...
function renderClawDevEnvNote(env: Record<string, string>): string {
  const clawdevKeys = Object.keys(env)
    .filter((key) => key.startsWith("CLAWDEV_"))
    .sort();
  if (clawdevKeys.length === 0) return "";
  return [
    "ClawDev runtime note:",
    `The following CLAWDEV_* environment variables are available in this run: ${clawdevKeys.join(", ")}`,
...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "CLAWDEV_API_URL") || !hasNonEmptyEnvValue(env, "CLAWDEV_API_KEY")) return "";
  return [
    "ClawDev API access note:",
    "Use run_shell_command with curl to make ClawDev API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $CLAWDEV_API_KEY\\" \\"$CLAWDEV_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $CLAWDEV_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-ClawDev-Run-Id: $CLAWDEV_RUN_ID\\" -d '{...}' \\"$CLAWDEV_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject ClawDev skills directly into `~/.gemini/skills/` via symlinks.
...
async function ensureGeminiSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[clawdev] Failed to prepare Gemini skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[clawdev] Removed maintainer-only Gemini skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[clawdev] ${result === "repaired" ? "Repaired" : "Linked"} Gemini skill: ${entry.key}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[clawdev] Failed to link Gemini skill "${entry.key}": ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geminiSkillEntries = await readClawDevRuntimeSkillEntries(config, __moduleDir);
  const desiredGeminiSkillNames = resolveClawDevDesiredSkillNames(config, geminiSkillEntries);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[clawdev] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const clawdevEnvNote = renderClawDevEnvNote(env);
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: clawdevEnvNote.length + apiAccessNote.length,
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[clawdev] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
