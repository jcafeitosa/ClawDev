...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="ClawDev API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "clawdevApiUrl",
                  String(config.clawdevApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "clawdevApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://clawdev.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "clawdev"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. ClawDev persists a device key during onboarding so pairing approvals
