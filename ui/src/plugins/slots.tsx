 ...
 *
 * Plugin bundles are built with `external: ["@clawdev/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__clawdevPluginBridge__`)
 ...
 * - `import { ... } from "@clawdev/plugin-sdk/ui";`
 *
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__clawdevPluginBridge__) {
 ...
 * self-register via `window.clawdevPlugins.registerReactComponent()`.
 ...
 * of `@clawdev/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
