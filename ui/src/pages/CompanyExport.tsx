...
/**
 * Filter .clawdev.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterClawDevYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://clawdev.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm clawdev company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [ClawDev](https://clawdev.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [ClawDev](https://clawdev.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .clawdev.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .clawdev.yaml
    const yamlPath = exportData.clawdevExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterClawDevYaml(exportData.files[yamlPath], checkedFiles);
    }
