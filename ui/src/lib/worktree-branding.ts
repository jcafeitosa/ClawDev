...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("clawdev-worktree-enabled") !== "true") return null;

  const name = readMetaContent("clawdev-worktree-name");
  const color = normalizeHexColor(readMetaContent("clawdev-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("clawdev-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
