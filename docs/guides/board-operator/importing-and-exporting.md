...
ClawDev companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .clawdev.yaml     # Adapter config, env inputs, routines
```
...
- **.clawdev.yaml** holds ClawDev-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
clawdev company export <company-id> --out ./my-export
```
...
clawdev company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
clawdev company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
clawdev company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.clawdev.yaml`
...
```sh
# From a local folder
clawdev company import ./my-export

# From a GitHub URL
clawdev company import https://github.com/org/repo

# From a GitHub subfolder
clawdev company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
clawdev company import org/repo
clawdev company import org/repo/companies/acme
```
...
If `--target` is not specified, ClawDev infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
clawdev company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
clawdev company import org/company-templates/engineering-team \
  --target new \
...
```sh
clawdev company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
clawdev company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
clawdev company import ./package \
  --target new \
  --yes \
  --json
```
...
ClawDev supports several GitHub URL formats:
